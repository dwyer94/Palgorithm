...
- **Owning the target never dead-ends the answer.** `cost = 0` for an owned target is the honest cheapest cost and is reported as such ("you already own this"), but it is *not* the whole result. Whenever the target is owned outright (baseline 0 combinations, no catches), the planner additionally computes a **next-best** plan on the roster with the owned copies of that species removed — the genuine cheapest way to obtain *another* one — and surfaces it alongside. This applies to the no-perk case and to owning a copy with the wrong (or even the exact) perks; the next-best plan carries its own perk overlay (§7.3) so "breed another with the perks I want" is answered too. The next-best plan is surfaced even when it turns out infeasible (with anchor-hint guidance), so ownership never hides *why* no other route exists.
...
- **2a. Opportunistic (zero added cost).** The cheapest species derivation, unmodified — identical combination count to Mode 1. Among final-cross parent choices *tied* at minimum cost, prefer whichever supplies the most of P for free (existing tie-break logic — this never adds a combination, it only chooses among already-equal-cost options). Per perk in P, report one of three states: "lands here on the final cross, X% odds"; "carried by an owned Pal already deeper in this tree — rides down for free with compounding per-generation odds, ~X% over N generations" (a real opportunistic path that costs no extra combinations, distinct from a flat "not sourced"); or "not present anywhere in this tree — only obtainable by chance during breeding, or by re-rolling/relocating afterward." This is always the cheapest possible answer; any certainty it offers is incidental, not engineered.
